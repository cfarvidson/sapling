...
- **`artifacts`** — markdown blobs. `kind TEXT` is free-form; canonical kinds used by Sapling skills include `review_notes`, `plan_review` (added in v0.12), `pending_questions`, `answers`, `architecture`, `scoping`, `dod_gaps`. Optional FKs to `work_item_id`, `plan_id`, `service_id` — all `ON DELETE SET NULL`. Multiple artifacts of the same kind per parent are allowed.
...
2. **`complete_work(id, ...)`** — if the completed item has a `project_id`, calls `advanceProjectAfterWorkCompletion(client, projectId, completedWork)` inside the same transaction. That helper fires these checks in order:
   ...
   - **DoD verifier** — if the completed item is a non-verifier and every non-verifier work item under the project is now completed, auto-enqueue one final `review` with `is_dod_verifier = true`, titled "Verify Definition of Done for project N: \<title\>." The re-arm guard is "no _pending / claimed / awaiting_input / blocked_ verifier exists" — completed-but-unverified verifiers (see DoD fix loop below) do **not** block re-arm.
...
### Seeded global teams (v0.12+)

Migration `013_seed_ce_teams.sql` ships two global teams whose roles point at [compound-engineering](https://github.com/every-com/compound-engineering) specialist agents, plus the matching global `team_defaults`. The seeded rows are not special — they are ordinary `teams` / `team_roles` / `team_defaults` rows that `/sapling:teams` can edit, delete, or shadow with a per-app team. All seed inserts are `ON CONFLICT DO NOTHING`, so user edits made before or after the migration are preserved.

| Team                       | Default for               | Roles point at (`subagent_type`)                                                                                                                                                                                                                                                                                                                                           |
| -------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sapling-code-review`      | `(NULL, 'review')` global | always-on: `ce-correctness-reviewer`, `ce-maintainability-reviewer`, `ce-testing-reviewer`, `ce-project-standards-reviewer`. conditional: `ce-security-reviewer`, `ce-data-migrations-reviewer`, `ce-schema-drift-detector`, `ce-performance-reviewer`, `ce-api-contract-reviewer`, `ce-reliability-reviewer`, `ce-previous-comments-reviewer`, `ce-adversarial-reviewer`. |
| `sapling-plan-stress-test` | `(NULL, 'plan')` global   | `ce-feasibility-reviewer`, `ce-scope-guardian-reviewer`, `ce-coherence-reviewer`, `ce-security-lens-reviewer`, `ce-design-lens-reviewer`, `ce-product-lens-reviewer`, `ce-adversarial-document-reviewer`.                                                                                                                                                                  |

### Dispatch matrix (lead behavior)

`/sapling:work` codifies which specialists fire for which work item shape. The matrix lives in `packages/claude-plugin/skills/work/SKILL.md`; this section is the spec-level recap.

- **`type='review'`** — lead dispatches the always-on four specialists in parallel, then any conditional specialist whose trigger matches the diff (path patterns, diff size, prior PR comments, etc.). When `is_dod_verifier=true`, the lead additionally dispatches `ce-adversarial-document-reviewer`, `ce-product-lens-reviewer`, and `ce-scope-guardian-reviewer` against the project's `definition_of_done_md`. The lead aggregates every specialist's findings into one `review_notes` artifact and computes the verdict from the merged blocker count.
- **`type='plan'`** — after `superpowers:writing-plans` produces a draft, the lead dispatches every specialist in the `sapling-plan-stress-test` role list against the draft body. Findings aggregate into one `plan_review` artifact. Blockers either get resolved by an inline plan edit (re-dispatch the affected specialists) or escalated via `request_human_input`; the plan is not persisted as `active` while blockers remain.

The DoD verifier work item, auto-enqueued by `complete_work` on the last non-verifier completion, picks up the same `team_defaults` resolution chain `enqueue_work` uses (per-app default → global default → null). Prior to v0.12 the auto-enqueued verifier always had `team_id = NULL`.

### Resilience

If a `ce-*` `subagent_type` listed on a seeded role is not present in the host's agent list (the compound-engineering plugin isn't installed), the lead skips that specialist and continues. Missing-agent state never fails the work item; the lead records skipped names in the internal-notes section of the artifact.
